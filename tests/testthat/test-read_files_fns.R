...
test_that("read_excelheader reads in header correctly", {
  ...
  header_test <- read_excelheader(sheets$sheet_name,
                                  path,
                                  sheets$header_start,
                                  sheets$header_end,
                                  unique_names = TRUE)
...
test_that("read_excelheader reads in header correctly (NAs and duplicate names)", {
  sheets <- list_sheetnames(data_folder, "test_header.xlsx") %>%
    mutate(header_start = 1) %>%
    mutate(header_end = 2)
  path <- paste(data_folder, sheets$filename, sep = "/")
  ans <- c("col", "col...2", "col_3", "...4")
  header_test <- read_excelheader(sheets$sheets, path, sheets$header_start, sheets$header_end, unique_names = TRUE)
...
test_that("read_excelheader reads in header correctly (NAs and duplicate names)", {
  ...
  header_test <- read_excelheader(sheets$sheet_name, path, sheets$header_start, sheets$header_end, unique_names = TRUE)
  expect_equal(ans, header_test$colnames)
