test_that("rm_dup_col removes duplicate columns of letters", {
  df1 <- data.frame(a = letters[1:2], b = letters[1:2], c = letters[3:4])
  ans <- df1 %>% select(a..b = a, c)
  expect_equal(rm_dup_col(df1), ans)
})

test_that("rm_dup_col removes duplicate columns and retains original names", {
  df1 <- data.frame(a = letters[1:2], b = letters[1:2], c = letters[3:4])
  ans <- df1 %>% select(a, c)
  expect_equal(rm_dup_col(df1, FALSE), ans)
})

test_that("rm_dup_col retains columns with different contents", {
  df1 <- data.frame(a = letters[1:2], c = letters[3:4])
  expect_equal(rm_dup_col(df1), df1)
})

test_that("rm_dup_col removes duplicate columns of letters (with more than one duplicate column)", {
  df1 <- data.frame(a = letters[1:2], b = letters[1:2], c = letters[3:4], d = letters[1:2])
  ans <- df1 %>% select(a..b..d = a, c)
  expect_equal(rm_dup_col(df1), ans)
})

test_that("rm_dup_col removes duplicate columns of numerics",{
  df1 <- data.frame(a = c(1,2,3), b = c(NA,2,3), c = c(NA,4,5), d = c(3, 4,5))
  ans <- df1 %>% select(a..b = a, d..c = d)
  expect_equal(rm_dup_col(df1), ans)
})

test_that("rm_dup_col detects if duplicate columns both contain NAs", {
...
# rm_dup_col() can handle the NAs in both cols c and d and removes column c
test_that("rm_dup_col detects if duplicate columns both contain NAs", {
...
test_that("rm_dup_col detects if duplicate columns both contain NAs", {
  ...
  expect_equal(rm_dup_col(df1), df1)
  expect_equal(rm_dup_col(df2), df2)
})
